"""
QUAD-task dataset: ISNT-order + per-quadrant rim ABN + CDR verdict + 5 SIGNS,
all on ONE image. Joins THREE manifests by id (verified: identical 733 ids,
identical split, train 613 / test 120, 0 mismatches):
  - isnt_manifest.csv        : rim_i/s/n/t (rank) + abn_i/s/n/t (3-cls per quad)
  - cdr_manifest.csv         : cdr_v_verdict / cdr_h_verdict (3-cls)
  - signs_manifest_clean.csv : lbl_{notch,rnfl,bayo,beta,ppa} (3-cls; dh dropped
                               — ~0% present). NA -> -1 (masked in signs loss).

Returns (image, rim[4]f, abn[4]long, cdr[2]long, sign[5]long(-1=NA), id, dataset).
Plain 224 resize, no augment, mean-rim-stratified val (seed=42) — same split
policy as dataset_tri / dataset_order_abn so all heads stay comparable.
"""
...
class QuadDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (x,
                ...
                torch.tensor(r["cdr"], dtype=torch.long),        # [2]
...
def build_dataloaders(batch_size=32, val_frac=0.10, seed=42, num_workers=4):
    ...
    info = {
        ...
        "cdr_train_class_counts": _multi_counts(train_rows, "cdr", N_CDR),
        "sign_train_class_counts": _multi_counts(train_rows, "sign", N_SIGN),
    }
    return (dl(QuadDataset(train_rows, PHOTOS_DIR, tf), True),
            dl(QuadDataset(val_rows, PHOTOS_DIR, tf), False),
            dl(QuadDataset(test_rows, PHOTOS_DIR, tf), False), info)


def _inv_freq_weights(counts, n_ch, n_cls, ignore_neg=False):
    import numpy as np
...
def sign_class_weights(info):
    # counts include key -1 (NA) which we just ignore (only read keys 0,1,2)
...
if __name__ == "__main__":
    ...
    print("sign counts:", info["sign_train_class_counts"])
    x, rim, abn, cdr, sign, ids, ds = next(iter(tr))
    print(f"x={tuple(x.shape)} rim={tuple(rim.shape)} abn={tuple(abn.shape)} "
          f"cdr={tuple(cdr.shape)} sign={tuple(sign.shape)}")
    print("sign0:", sign[0].tolist(), "id0:", ids[0])
    print("sign weights:\n", sign_class_weights(info))
