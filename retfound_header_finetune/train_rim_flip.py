...
from dataset_rim_flip import (build_dataloaders, abn_class_weights,
...
def evaluate(model, loader, device, tie_eps):
    model.eval()
    S, R, AL, AP, D = [], [], [], [], []
    for x, rim, abn, _ids, ds in loader:
        x = x.to(device)
        s, al = model(x)
...
def main():
    ...
    tag = f"rimflip_{args.pool}_{args.head}_{ts}"
    ...
    model = JointModel(backbone, pool, order_head, abn_head, lora=args.lora).to(device)
    ...
    for ep in range(args.epochs):
        ...
        for x, rim, abn, _i, _d in tr:
            x, rim, abn = x.to(device), rim.to(device), abn.to(device)
            scores, abn_logits = model(x)
